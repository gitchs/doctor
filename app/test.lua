...
CREATE TABLE IF NOT EXISTS m2(
    ...
    delegated_user VARCHAR(255) DEFAULT NULL,
    connected_user VARCHAR(255) DEFAULT NULL,
...
local function cli_init()
    optlib.define('--help', '-h', {nargs=0})
    optlib.define('--save_profile', '-p', {nargs=0})
    optlib.define('--avro_file', '-f')
    optlib.define('--database_file', '-d')
    optlib.parse(arg)

    if optlib.get('help') then
...
local function main()
    ...
    for row in iterators.avro(cli_configure.avro_file) do
        ...
        local db_row = {
            ...
            user = summary:info_strings('User'),
            connected_user = summary:info_strings('Connected User'),
            delegated_user = summary:info_strings('Delegated User'),
